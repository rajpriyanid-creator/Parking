...
exports.searchParkings = async (req, res) => {
    try {
        ...
        // Batch fetch slots to resolve N+1 queries
        const parkingIds = results.map(r => r.parking._id);
        const allSlots = await Slot.find({ parkingId: { $in: parkingIds } });

        const slotsByParking = {};
        for (const slot of allSlots) {
            const pId = slot.parkingId.toString();
            if (!slotsByParking[pId]) slotsByParking[pId] = [];
            slotsByParking[pId].push(slot);
        }

        // Annotate with live slot counts synchronously
        const annotated = results.map(({ parking, distanceMeters }) => {
            const slots = slotsByParking[parking._id.toString()] || [];
...
exports.getAllParkings = async (req, res) => {
    try {
        ...
        // Batch fetch slots
        const parkingIds = allParkings.map(p => p._id);
        const allSlots = await Slot.find({ parkingId: { $in: parkingIds } });
        ...
        const result = allParkings.map((parking) => {
            const slots = slotsByParking[parking._id.toString()] || [];
            const available = slots.filter(s => s.status === 'AVAILABLE').length;
            const evAvailable = slots.filter(s => s.status === 'AVAILABLE' && s.slotType === 'EV').length;
            const bikeAvailable = slots.filter(s => s.status === 'AVAILABLE' && s.slotType === 'BIKE').length;
...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        // Filter nearby parkings first
        const nearbyParkings = allParkings.map(p => {
            const dist = haversine(lat, lng, p.location.lat, p.location.lng);
            return { parking: p, dist };
        }).filter(p => p.dist <= SUGGESTION_RADIUS_M);

        const parkingIds = nearbyParkings.map(p => p.parking._id);
        const allSlots = await Slot.find({ parkingId: { $in: parkingIds } });
        ...
        const suggestions = nearbyParkings.map(({ parking, dist }) => {
            const slots = slotsByParking[parking._id.toString()] || [];
            const available = slots.filter(s => s.status === 'AVAILABLE').length;
            const evAvail = slots.filter(s => s.status === 'AVAILABLE' && s.slotType === 'EV').length;
            const minPrice = slots.reduce((min, s) => s.basePricePerHour < min ? s.basePricePerHour : min, Infinity);
            return { parking, distanceMeters: Math.round(dist), availableSlots: available, evAvailableSlots: evAvail, minPricePerHour: minPrice === Infinity ? 0 : minPrice };
        });

        const filtered = suggestions.filter(s => s.availableSlots > 0).sort((a, b) => a.distanceMeters - b.distanceMeters);
        res.json(filtered);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
